...
class PyAgriculture:
    """Main API for reading and converting TASKDATA/TLG files.

    Usage: create an instance with the path to a TaskData folder and call
    `gather_data()` or `gather_task_names()` to parse available tasks.
    Attributes such as `tasks` and `task_infos` are populated after parsing.
    """
    ...
    def _check_path_is_ok(self):
        """Ensure the provided path contains `TaskData.xml` and normalize it."""
        if self.path[-1] not in ['/', '\\']:
            self.path += '/'
        # Accept any case variant of the TaskData filename (case-insensitive)
        p = Path(self.path)
        taskdata_name = None
        try:
            for entry in p.iterdir():
                if entry.is_file() and entry.name.lower() == 'taskdata.xml':
                    taskdata_name = entry.name
                    break
        except OSError:
            taskdata_name = None
        if taskdata_name is None:
            raise FileNotFoundError(f"The specified path '{self.path}' does not contain a TaskData.xml file")
        # Save actual filename for later parsing (preserve original casing)
        self._taskdata_filename = taskdata_name
    ...
    def gather_task_names(self, continue_on_fail=True) -> list:
        """Parse available TLG files and return a list of human-readable task names.

        This method reads `TASKDATA.xml`, walks referenced TLG XML files and
        collects minimal info (used by UIs to populate choices) without
        loading full binary data.
        """

        task_data_dict = {}
        task_names = []
        tree = ET.parse(self.path + self._taskdata_filename)
    ...
    def gather_data(self, most_important='dry yield', continue_on_fail=True, only_tasks=[]):
        """Parse TASKDATA and associated TLG/BIN files and populate `self.tasks`.

        Parameters
        - most_important: name of the column used as a filter for rows to keep
        - continue_on_fail: if False, missing files raise exceptions
        - only_tasks: optional list of task names to include
        """
    ...
    def set_ptn_data(self, tlg_dict: dict) -> None:
        """Set the numpy dtype (`self.dt`) and static byte size for PTN records.

        Inspects the `tlg_dict['PTN']` keys to determine which GPS/position
        fields are present and builds the corresponding structured dtype.
        """
    ...
    def get_tlg_columns(tlg_dict) -> list:
        """Return a list of output column names for a TLG dictionary.

        The returned list contains basic time/position columns plus one
        column per `DLV` device with a `Name` attribute.
        """
    ...
    def read_binaryfile(self, file_path: str, tlg_dict: dict, df_columns: list, most_important: str,
        ...
        """Read a `.bin` file and convert it to a pandas DataFrame.

        Uses the configured `self.read_with_cython` flag to select a Cython
        implementation where available for performance. Returns a DataFrame
        with metadata in `df.attrs` or `None` if no rows matched.
        """
        ...
        while read_point < len(binary_data):
            ...
            if self.read_with_cython:
                # use the preferred cython implementation when available; the
                # module-level import at the top of this file falls back to the
                # pure-Python implementation if the compiled extension is
            ...
            for idx, col_name in enumerate(df_columns):
                if idx >= nr_static:
                    try:
                        ...
                        # If unit info isn't available, leave the column name unchanged
                        pass
    ...
    def read_dlvs(binary_data, read_point: int, nr_dlvs: int, nr_static: int, dpd_ids: dict,
        ...
        for nr, dlv in np.frombuffer(binary_data, [('DLVn', np.dtype('uint8')),
            ...
            if dpd_key in dpd_ids.keys():
                ...
                continue
            # If there's no DVP (scaling/unit) information, still keep the
            # raw DLV value instead of skipping it so properties appear.
